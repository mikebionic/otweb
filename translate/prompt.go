...
// DefaultPromptTemplate - промпт перевода названия+описания на RU+TK + оценка веса.
// Фаза 0 экономии токенов (30.07): убраны 18 нормализованных характеристик и keywords —
// они гейтились pushDeepSeekFeatures=false, не сохранялись и не пушились (мёртвая генерация).
// Характеристики CS-Cart идут из attr_cs_mapping, а не отсюда. Осталось только реально
// используемое: title_ru/tk, description_ru/tk, estimated_weight_grams.
const DefaultPromptTemplate = `Ты - переводчик для e-commerce маркетплейса. Переведи название и описание товара с китайского маркетплейса на 2 языка (русский, туркменский) и оцени вес. Верни ТОЛЬКО JSON.
